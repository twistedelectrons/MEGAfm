...
void lfoTick() {
  for (int i = 0; i < 3; i++) {
    if ((i == 0) && (lfoVel) && (fmBase[36] == 0)) {}//
      ...
      switch (lfoShape[i]) {
        case 0:
          if (invertedSquare[i]) { if (lfoStep[i] < 127) { lfo[i] = 0; } else { lfo[i] = 255; }}
          else { if (lfoStep[i] < 127) { lfo[i] = 255; } else { lfo[i] = 0; }}
          break;//square

        case 1:
          if (lfoStep[i] < 128) { lfo[i] = lfoStep[i] << 1; }
          else { lfo[i] = 255 - (lfoStep[i] << 1); }
          lfo[i] = lfo[i];
          break;//triangle

        case 2:
          if (!invertedSaw[i]) { lfo[i] = lfoStep[i]; }
  ...
  for (int i = 0; i < 3; i++) {
    if ((i == 0) && (lfoVel) && (fmBase[36] == 0)) {}//
      ...
      while (lfoCounter[i] >= 512) {
        lfoCounter[i] -= 512;
        if (lfoStep[i] < 255) { lfoStep[i]++; }
        else if (lfoStep[i] == 255) {
          if (looping[i]) {
            if (i == selectedLfo) { lfoBlink(); }
            notRandomed[i] = true;
            lfoStep[i] = 0;
          } else { lfoStep[i] = 255; }
        }
...
byte getRandom(byte number) {
  if ((looping[number]) && (noiseTableLength[number] != 2)) {
    randomIndex[number]++;
    if (randomIndex[number] > 1 << noiseTableLength[number]) { randomIndex[number] = 0; }
