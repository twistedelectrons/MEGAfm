...
if(lfoAt){if(fmBase[40]){fmBase[41]=val<<1;fmBaseLast[41]=fmBase[41]-1;ledNumberTimeOut=20;}else{atDest=val<<1;ledNumberTimeOut=20;}}
}
...
vibIndex=int(vibIndexF);
  ...
  if(absoluteClockCounter>=48){
    leftDot();absoluteClockCounter=0;

    if(arpClockEnable){
    if((arpMidiSpeed!=arpMidiSpeedPending)||(resyncArp)){
      arpMidiSpeed=arpMidiSpeedPending;arpClockCounter=0;
      resyncArp=false;
    }}


for(int i=0;i<3;i++){if(lfoClockEnable[i]){if(lfoClockSpeedPending[i]){lfoClockSpeed[i]=lfoClockSpeedPending[i]-1;lfoClockSpeedPending[i]=0;lfoStepF[i]=0;}}}
...
///////////////////////////////////
  ...
  if((arpClockEnable)&&(arpMode)&&(voiceMode==3)&&(arpMode!=7)){
    arpClockCounter++;if((arpClockCounter>=midiArpTicks[arpMidiSpeed])){arpClockCounter=0;if(!emptyStack){arpFire();}}}
...
if((lfoClockEnable[0])||(lfoClockEnable[1])||(lfoClockEnable[2])){
  ...
  syncLfoCounter++;
  if(syncLfoCounter==24){syncLfoCounter=0;
  }
  ...
  if(lfoStepF[i]>=255){if(looping[i]){if(selectedLfo==i){lfoBlink();}lfoStepF[i]=0;lfoNewRand[i]=1;}else{lfoStepF[i]=255;}}
...
if(mpe){
  ...
  bendy=0;
  if(bend!=0){
    if(bend<0){mpeBend[channel-1]=bend;mpeBend[channel-1]/=8192;mpeBend[channel-1]*=bendDown;}else if(bend>0){mpeBend[channel-1]=bend;mpeBend[channel-1]/=8191;mpeBend[channel-1]*=bendUp;}
 }
 setNote(channel-1,notey[channel-1]);
  ...
  //-8192 to 8191
  bendy=0;
  if(bend!=0){if(bend<0){bendy=bend;bendy/=8192;bendy*=bendDown;}else if(bend>0){bendy=bend;bendy/=8191;bendy*=bendUp;}}
...
if(setupMode){
  ...
  if(bendRoot==-1){bendRoot=note;


  digit(0,10);digit(1,23);delay(500);lastNumber=-1;ledNumber(inputChannel);delay(750);
  ...
  if(channel!=inputChannel){
    ...
    if(note>bendRoot){bendUp=note-bendRoot;if(bendUp>48){bendUp=48;}EEPROM.write(3959,byte(bendUp));

    digit(0,13);digit(1,14);delay(500);lastNumber=-1;ledNumber(bendUp);delay(750);}

    else if(note<bendRoot){bendDown=bendRoot-note;if(bendDown>48){bendDown=48;}EEPROM.write(3958,byte(bendDown));

    digit(0,15);digit(1,19);delay(500);lastNumber=-1;ledNumber(bendDown);delay(750);}
       }

}else{

if(lfoVel){if(fmBase[36]){fmBase[37]=velocity<<1;fmBaseLast[37]=fmBase[37]-1;dontShow[37]=1;}else{lfo[0]=velocity<<1;applyLfo();}}
...
//reset ARP
if(arpMode==6){arpStep=seqStep=0;arpIndex=0;}
...
if(glide){setNote(voiceSlot,lastNotey[heldKeys]);skipGlide(voiceSlot);}
...
voiceSlot++;if(voiceSlot>11)voiceSlot=0;
...
//if gliding jump to last pitch associated to keycounter 
if(glide){
  setNote(voiceSlot,lastNotey[heldKeys]);skipGlide(voiceSlot);
  setNote(voiceSlot+6,lastNotey[heldKeys]);skipGlide(voiceSlot+6);}
noteOfVoice[voiceSlot]=note;
lastNotey[heldKeys]=note;
setNote(voiceSlot,noteOfVoice[voiceSlot]);setNote(voiceSlot+6,noteOfVoice[voiceSlot]);
ym.noteOff(voiceSlot);ym.noteOff(voiceSlot+6);
ym.noteOn(voiceSlot);ym.noteOn(voiceSlot+6);

voiceSlot++;if(voiceSlot>5)voiceSlot=0;
...
//fire at the same time
    ...
    noteToChannel[note]=4;ym.noteOff(4);setNote(4,note);ym.noteOn(4);//CHIP1
    setNote(2,note);
    
   noteToChannel[note]=5;ym.noteOff(5);setNote(5,note);ym.noteOn(5);//CHIP2
   setNote(8,note);
  }else{
...
if(((pedal)&&(arpClearFlag))||(heldKeys==1)){
  arpClearFlag=false;
  clearNotes();heldKeys=1;
}

if((heldKeys==1)&&(!pedal)&&(!sync)){arpCounter=1023;}//only retrigger arp on first key or if arp is stopped
...
seq[seqLength]=note-rootNote1+127;if(seq[seqLength]==255){seq[seqLength]--;}if(seqLength<16){seqLength++;ledNumber(seqLength);}
...
}else{handleNoteOff(channel, note, velocity);}leftDot();
}}}}
...
////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////
  case 0:// poly12
  ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////   ////
heldKeys--;if(heldKeys<0)heldKeys=0;

  if(pedal){
  //find out which channel the note is and add to pedalOff
   for(int i=0;i<12;i++){
if(noteOfVoice[i]==note){pedalOff[i]=1;//abort loop
}}
...
for(int i=0;i<12;i++){
if((voiceSlots[i])&&(noteOfVoice[i]==note)){voiceSlots[i]=0;ym.noteOff(i);
...
heldKeys--;if(heldKeys<0)heldKeys=0;
  ...
  //find out which channel the note is and add to pedalOff
   for(int i=0;i<6;i++){
if(noteOfVoice[i]==note){pedalOff[i]=1;pedalOff[i+6]=1;//abort loop
}}
...
voiceSlots[i]=0;ym.noteOff(i);
voiceSlots[i+6]=0;ym.noteOff(i+6);
...
if(!pedal){for(int i=0;i<12;i++){ym.noteOff(i);}}
  ...
  switch(notePriority){
  case 0:note=getLow();break;//LOWEST
  case 1:note=getHigh();break;//HIGHEST
  case 2:note=getLast();break;//LAST
}
  rootNote=note;for(int i=0;i<12;i++){setNote(i, note);}
...
void sendCC(byte number,int value){
  ...
  if(lastCC[number]!=value){lastCC[number]=value;
  rightDot();
...
if((number==0)){if(val<5){if(bank!=val){bank=val;HandlePc(inputChannel,preset-1);}}}
  ...
  if(lfoMod){if(fmBase[38]){fmBase[39]=val<<1;fmBaseLast[39]=fmBase[39]-1;ledNumberTimeOut=20;}else{lfo[1]=val<<1;applyLfo();}}}


else if((number==7)){if(allCC){movedPot(1,val<<1,1);}}
else if(number==64){if(val>63){pedalDown();}else{pedalUp();}
  }else{

   if(allCC){movedPot(number,val<<1,1);}else{ 
...
for(int i=0;i<12;i++){
  if(pedalOff[i]){
   ...
   ym.noteOff(i);pedalOff[i]=0;if(heldKeys){heldKeys--;}
break;//poly
...
//SEND FINE!!!!!!!!!!!!
  break;//volume
    ...
    case 28:
    //SEND GLIDE!!!!!!!!!!!!
    sendCC(number,fmBase[50]>>1);break;//fat 1-127
