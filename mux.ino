 ...
 void readMux(){
   ...
   muxChannel++;if(muxChannel>15){muxChannel=0;}mux(muxChannel);
...
int pot;
  ...
  {potLast[muxChannel]=pot;movedPot(muxChannel,pot>>1,0);}
  ...
  {potLast[muxChannel+16]=pot;movedPot(muxChannel+16,pot>>1,0);}
   
  pot=analogRead(A3)>>1;
  if((pot>potLast[muxChannel+32]+range)||(pot<potLast[muxChannel+32]-range))
  
  {potLast[muxChannel+32]=pot;movedPot(muxChannel+32,pot>>1,0);}
...
if(muxChannel==15){
  ...
  {potLast[48]=pot;movedPot(48,pot>>1,0);}
}
  ...
  //butts 
  pot=digitalRead(A1);
  if(pot!=buttLast[muxChannel]){buttLast[muxChannel]=pot;buttChanged(muxChannel,buttLast[muxChannel]);}

  if(muxChannel==15){

  pot=digitalRead(22);
  if(pot!=buttLast[18]){buttLast[18]=pot;buttChanged(18,buttLast[18]);}
  }
